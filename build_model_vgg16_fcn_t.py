...
from keras.layers import Input, Conv2D, Conv2DTranspose, Activation, MaxPooling2D, UpSampling2D, Reshape, add, multiply, concatenate, Lambda, ZeroPadding2D, Conv3D, Add
...
def model_calc(lam1, lam2):
    ...
    deconv1 = Conv2DTranspose(filters=512,kernel_size=(2,2), strides=(2,2),activation='relu',data_format='channels_last', kernel_regularizer=regularizers.l2(lam2), activity_regularizer=regularizers.l1(lam1))(conv)
    skip1 = Add()([model_vgg2.get_layer('block5_conv3').output, deconv1])
    ...
    skip2 = Add()([model_vgg2.get_layer('block4_conv3').output, deconv2])
    ...
    skip3 = Add()([model_vgg2.get_layer('block3_conv3').output, deconv3])
    ...
    skip4 = Add()([model_vgg2.get_layer('block2_conv2').output, deconv4])
    ...
    skip5 = Add()([model_vgg2.get_layer('block1_conv2').output, deconv5])
...
if __name__ == '__main__':
    ...
    trnamestr = os.path.join(args.output_dir, 'predtr_ex_fcn_t_reg' + str(args.lam1) + '_' + str(args.lam2) + '_' + namestr + '.png')
    plt.imsave(trnamestr, predtr_ex, cmap = plt.get_cmap('gray'))
    trnamestr_lab = os.path.join(args.output_dir, 'labtr_ex_fcn_t_reg' + str(args.lam1) + '_' + str(args.lam2) + '_' + namestr + '.png')
    ...
    valnamestr = os.path.join(args.output_dir, 'predval_ex_fcn_t_reg' + str(args.lam1) + '_' + str(args.lam2) + '_' + namesval + '.png')
    plt.imsave(valnamestr, predval_ex, cmap = plt.get_cmap('gray'))
    valnamestr_lab = os.path.join(args.output_dir, 'labval_ex_fcn_t_reg' + str(args.lam1) + '_' + str(args.lam2) + '_' + namesval + '.png')
    ...
    iounamestr = os.path.join(args.output_dir, 'iou_fcn_t_reg' + str(args.lam1) + '_' + str(args.lam2) + '_' + namestr + '.png')
    ...
    iounamestr = os.path.join(args.output_dir, 'mis_fcn_t_reg' + str(args.lam1) + '_' + str(args.lam2) + '_' + namestr + '.png')
    plt.savefig(iounamestr)
    ...
    lossnamestr = os.path.join(args.output_dir, 'loss_fcn_t_reg' + str(args.lam1) + '_' + str(args.lam2) + '_' + namestr + '.png')
    plt.savefig(lossnamestr)
    
    wnamestr = os.path.join(args.output_dir, 'w_fcn_t_reg' + str(args.lam1) + '_' + str(args.lam2) + '_' + namestr + '.h5')
